...
async def _write_redis(session_id: str, user_msg: dict, bot_msg: dict):
    r = get_redis()    
    key = f"chat:{session_id}:recent"
    # 리스트에 최근 20개만 유지 
    await r.rpush(key, json.dumps(user_msg), json.dumps(bot_msg))
    await r.ltrim(key, -40, -1)  # user/bot 한 쌍 기준 20개

async def persist(state: ChatState) -> ChatState:
    msgs = state["messages"]
    if len(msgs) == 0:
        return state
    user_m = msgs[-2] if len(msgs) >= 2 else {"role":"user","content":""}
    bot_m  = msgs[-1]
    ...
    try:
        ...
        await _write_pg(
            ...
            user_msg=user_m.get("content",""),
            bot_msg=bot_m.get("content",""),
        )
    except Exception as e:
        # 저장 실패는 상태에만
        state["error"] = (state.get("error") or "") + f" [pg:{type(e).__name__}:{e}]"
    ...
    try:
        ...
        state["error"] = (state.get("error") or "") + f" [redis:{type(e).__name__}:{e}]"
